...
Select a service row to open its latest 100 journal lines and follow new entries in the configured terminal. If the terminal cannot be opened, the desklet copies the scoped `journalctl` command to the clipboard and tells you.
...
The refresh interval, notifications, background, transparency, and text size are configurable. This desklet monitors only local, continuous systemd services. It does not monitor remote hosts, timers, containers, or whole-machine outages.
