...
Stopped rows render yellow, failed rows and systemd load errors render red, and missing or unavailable rows remain neutral. The status text still carries the meaning, so color is a cue rather than the only signal.
